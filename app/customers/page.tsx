  ...
  useEffect(() => {
    const fetchCustomersData = async () => {
      ...
      const { data: ordersData } = await supabase
        .from('orders')
        .select('customer_name, customer_phone, payment_method, total_price, created_at, status')
      ...
      if (ordersData) {
        ...
        ordersData.forEach((order) => {
          ...
          const date = order.created_at
            ...
            : 'N/A';
          const entry: OrderEntry = { status, total_price: price, created_at: date, payment_method: paymentMethod };
              ...
              {filteredCustomers.map((customer, index) => (
                <div key={index} className="bg-[#09090b] border border-zinc-800 rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-1 rounded-md text-[11px] font-medium uppercase tracking-wide ${statusStyle(customer.latestStatus)}`}>
                  ...
                  {expandedCustomer === customer.name && (
                    <div className="border-t border-zinc-800 pt-3 space-y-2">
                      ...
                      {customer.orders.map((o, i) => (
                        <div key={i} className="flex items-center justify-between text-xs bg-zinc-900/60 border border-zinc-800/60 rounded-lg px-3 py-2">
